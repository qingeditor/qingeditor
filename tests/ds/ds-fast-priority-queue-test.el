...
(ert-deftest qingeditor/test/ds/fats-priority-queue-get-priority-list-test ()
  ...
  (qingeditor/test/ds/prepare-fast-priority-queue
    (lambda ()
    (let (list)
      (setq list (qingeditor/cls/get-priority-list queue -1))
      (should (not (eq list
		       (qingeditor/cls/get (oref queue :values) -1))))
...
(ert-deftest qingeditor/test/ds/fast-priority-queue-remove-test ()
  ...
  (qingeditor/test/ds/prepare-fast-priority-queue
   (lambda ()
      (qingeditor/cls/remove queue "test3")
      (let (list)
      (setq list (qingeditor/cls/get-priority-list queue -1))
      (should (equalp list '("test4")))
      (should (equal nil (qingeditor/cls/remove queue "test3")))
      (should (equal t (qingeditor/cls/remove queue "test4")))
      (setq list (qingeditor/cls/get-priority-list queue -1))
...
(ert-deftest qingeditor/test/ds/fast-priority-queue-empty-test ()
  ...
  (lambda ()
    (let (new-queue)
      (should (eq (qingeditor/cls/empty queue) nil))
      (setq new-queue (qingeditor/fast-priority-queue))
      (should (eq (qingeditor/cls/empty new-queue) t)))))


(ert-deftest qingeditor/test/ds/fast-priority-queue-contains-test ()
  :tags '(qingeditor/fast-priority-queue/contains)
  (lambda ()
    (should (eq (qingeditor/cls/contains queue "test9") nil))
    (should (eq (qingeditor/cls/contains queue "test3") t))
    (should (eq (qingeditor/cls/contains queue "test4") t))))
