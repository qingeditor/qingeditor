...
(defun qingeditor/c-c++/find-clang-complete-file ()
...
;; Based on the Sarcasm/irony-mode compilation database code.
(defun qingeditor/c-c++/load-clang-complete-file (cc-file)
...
(defun qingeditor/c-c++/load-clang-args ()
...
and the arguments for flyckeck-clang based on a project-specific text file."
  (unless company-clang-arguments
    (let* ((cc-file (qingeditor/c-c++/find-clang-complete-file))
           (flags (if cc-file (qingeditor/c-c++/load-clang-complete-file cc-file)
                    '()))
           (dirs (mapcar (lambda (f) (substring f 2))
                         (remove-if-not (lambda (f) (string-prefix-p "-I" f)) flags))))
      (setq-local company-clange-arguments flags)
      (setq-local company-c-headers-path-system
                  (append '("/usr/include" "usr/local/include") dirs))
